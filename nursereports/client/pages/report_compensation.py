...
def editing() -> rx.Component:
    return rx.cond(
        ...
        rx.callout(
            rx.text(
                f"You are currently editing a previously submitted report for {ReportState.hospital_info['hosp_name']}"
            ),
            icon="info",
            class_name="w-full",
        ),
    ...
    return flex(
        ...
        rx.flex(
                    ...
                    rx.flex(
                        ...
                        rx.flex(
                            rx.flex(
                                ...
                                on_click=ReportState.set_comp_check_benefit_reimbursement(
            ...
            rx.flex(
                rx.flex(
                    rx.text(
                        "Comments or additional info for your nursing peers about pay or benefits? (Optional)"
                    ),
                    rx.flex(
                        rx.cond(
...
def calculator(label: str) -> rx.Component:
    return rx.popover.root(
        rx.popover.trigger(
            rx.flex(
                rx.icon("pencil", class_name="h-5 w-5 dark:stroke-zinc-500"),
                on_click=[
