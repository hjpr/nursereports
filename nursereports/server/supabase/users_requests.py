...
def supabase_create_initial_user_info(access_token: str, user_id: str) -> None:
    ...
    user_info = {
        "id": user_id,
        "modified_at": None,
        "last_login": str(datetime.now(timezone.utc).isoformat(timespec="microseconds")),
        "account": {
            "created_at": str(datetime.now(timezone.utc).isoformat(timespec="microseconds")),
            "status": "onboard",
            "trust": 0,
            "membership": "free",
            "browsers": []
...
def supabase_update_user_info(
    ...
    """
    data["modified_at"] = str(datetime.now(timezone.utc).isoformat(timespec="microseconds"))
...
def supabase_update_last_login(
    ...
    """
    data = {}
    data["last_login"] = str(datetime.now(timezone.utc).isoformat(timespec="microseconds"))
    url = f"{api_url}/rest/v1/users?id=eq.{user_id}"
    headers = {
        "apikey": api_key,
        "Authorization": f"Bearer {access_token}",
